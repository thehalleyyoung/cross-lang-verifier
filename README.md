...
[![Three-minute demo: c2rust CWE-369-class divergence confirmed by real compilers](docs/assets/demo_video_poster.png)](docs/assets/demo_video.mp4)

The linked three-minute demo is regenerated by `python3 scripts/build_demo_video.py`.
It confirms a checked-in c2rust `nginx_rate` extraction unit against real
`clang`/UBSan + `rustc`, then teases the same CWE class on C→Go.
...
- **proof-carrying counterexamples**: every confirmed source-UB witness now ships
  ...
  inference; minimized witnesses carry the same scoped preservation proof, and
  cached divergences preserve the certificate
- a **soundness-regression CI gate** (`make soundness-check`) that enumerates all
  ...
  probe; `docs/SOUNDNESS_COMPENDIUM.md` is the generated oracle→theorem→witness
  map
...
- a stdio **Language Server Protocol diagnostics adapter** (`cross-lang-verify-lsp`)
  that turns the same manifest-backed verifier findings into inline editor
  warnings/errors without fabricating source locations
...
- deterministic parallel, sharded, and distributed-manifest ground-truth
  ...
  repro-hardening-check` also quarantines flaky compiler/runtime evidence and
  validates the result-store v2 reproducibility lemma
