...
recursive-descent frontends, lowers both to a shared typed SSA IR, constructs
product programs for aligned function pairs, and verifies equivalence via SMT
solving with concrete counterexample extraction.

We evaluate \textsc{SemRec} on three experiments:
(1)~a \textbf{core benchmark} of 32 hand-crafted C$\leftrightarrow$Rust pairs
achieving 100\% accuracy (32/32) with zero unknowns, all 16 divergences
detected with Z3-extracted counterexamples, and a 100\% pipeline success rate
(32/32 product programs constructed);
(2)~a \textbf{real-world experiment} on 50 C functions from common systems
patterns (hashing, bit manipulation, overflow-safe arithmetic, string utilities)
translated by GPT-4.1-nano, where \textsc{SemRec} finds 74 semantic bugs across
37/50 translations---outperforming random testing (34/50) and
no-$\sigma$-bridge bitvector comparison (19/50);
(3)~an \textbf{ablation study} confirming the $\sigma$-bridge is necessary
...
\begin{enumerate}[leftmargin=*]
  \item The \textbf{$\sigma$-bridge} semantic reconciliation framework that
    encodes per-language arithmetic semantics into SMT queries
  ...
  \item \textbf{Three experiments} demonstrating 100\% accuracy on core
    benchmarks, 74 real bugs found in LLM translations of 50 real-world
    C functions, and ablation confirming the $\sigma$-bridge's necessity
...
\begin{theorem}[Three-Output Exhaustiveness]\label{thm:three-output}
...
\end{theorem}
\begin{proof}
Follows from Z3's trichotomy: \texttt{sat}, \texttt{unsat}, or
...
We evaluate \textsc{SemRec} on three experiments designed to answer:
\begin{enumerate}
  ...
  \item[\textbf{RQ3}] Is the $\sigma$-bridge necessary, or do simpler
...
\subsection{Ablation Study (RQ3)}\label{sec:eval-ablation}
...
\begin{itemize}
  ...
  \item Loop-free or bounded-loop functions (Z3 bitvector theory is
    decidable; unbounded loops require loop invariants).
  \item Primitive types: integers (signed/unsigned, 8--64 bits) and
    IEEE 754 floats.  Struct and pointer verification is limited
    to the IR level and not exercised in the SMT encoding.
  \item Functions of $\sim$20 lines.  Scaling to larger functions
    requires compositional verification (future work).
\end{itemize}

The benchmark functions are small-scale, representative of common
utility code.  Scaling to full-program verification (with heap, function
pointers, and concurrency) is future work.  However, the current scope
covers the most common and dangerous divergence class: signed integer
overflow, which accounts for 50\% of all bugs found.
...
On 32 hand-crafted benchmarks, \textsc{SemRec} achieves 100\% accuracy
with 100\% pipeline success.  On 50 real-world C functions translated by
GPT-4.1-nano, \textsc{SemRec} finds 74 genuine semantic
bugs---outperforming random testing (34/50 detected vs.\ 37/50) and
no-$\sigma$-bridge analysis (19/50).  The $\sigma$-bridge is essential:
without it, accuracy drops from 100\% to 30\%.

\textsc{SemRec} is implemented in $\sim$100K lines of Python and is
available as open-source software.  The modular pipeline (parsers, IR,
product programs, SMT solver) can be extended to additional language
pairs and semantic domains.
