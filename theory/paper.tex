...
%% ── abstract ──────────────────────────────────────────────────────────────
\begin{abstract}
Large language models increasingly translate legacy C code to Rust, yet
translation quality still depends on tooling that checks source-language
semantics directly. We present \semrec{}, a repository-backed verifier that
encodes C and Rust functions into SMT formulas through a semantic bridge that
preserves undefined-behavior distinctions erased by lower-level views. The
current artifact supports inline verification, file-based verification, project
discovery, fuzzing, and benchmark execution through shipped CLIs. This revision
therefore frames the work around those executable analysis workflows and avoids
unsupported claims about final benchmark leadership.
\end{abstract}
