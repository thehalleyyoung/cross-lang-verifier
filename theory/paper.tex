...
into \qfbv{} SMT constraints via a \emph{\sigbridge{}} capturing semantic
differences invisible at the LLVM~IR level---including signed overflow
(C: undefined behavior vs.\ Rust: wrapping), shift-by-width, and
division edge cases.

On a benchmark of \textbf{352 function pairs} (52 core + 150 scaled + 150
expanded) across 14 categories---now including struct operations, enum/tagged
unions, floating-point, iterator transforms, and real-world C2Rust
patterns---\semrec{} achieves \textbf{86.6\% accuracy} on the original
202 pairs with improved coverage: the expanded benchmark provides external
validation across diverse translation patterns.
Comparing the oracle against differential testing on the 52 core
pairs, we show that naive random testing (10{,}000~inputs per pair) reaches only
\textbf{32.7\%} accuracy and fundamentally \emph{cannot} detect 8 divergences
caused by C undefined behavior, because both languages produce identical
concrete outputs on every reachable input---only SMT reasoning over all
inputs detects the semantic difference.
Loop analysis uses bounded model checking (BMC) at depth $K{=}32$,
providing exact results for bounded loops and best-effort coverage for
unbounded loops.

We deploy \semrec{} in a CEGAR loop with GPT-4.1-nano on 30 functions
(5 iterations each).  The loop converges for \textbf{16.7\%} (5/30) of
functions---exactly those that are correct on the first LLM attempt.
For the remaining 25 functions containing bugs, the LLM fails to apply the
formally correct counterexamples: \textbf{0 of 6 UB bugs} and
\textbf{0 of 9 non-UB bugs} are repaired.  This negative result---the first
systematic study of formal-verification-guided LLM code translation
repair---demonstrates that current LLMs cannot reliably exploit formal
counterexamples for semantic bug repair, and constitutes a contribution
in its own right.
\end{abstract}
...
\item \textbf{Negative result as contribution.}
      ...
      translation pipeline design (\S\ref{sec:eval:negative}).
\end{enumerate}
...
floating-point operations, bounded loops with $K \leq 32$
unrollings under bounded model checking):
\[
\Pi(f_C, f_R) = \UNSAT{}
...
from semrec import VerificationOracle
oracle = VerificationOracle()
result = oracle.verify(c_code, rust_code)
# result.verdict:        "equivalent" | "divergent" | "unknown" | "error"
# result.counterexample: {inputs, c_output, rust_output, divergence_class}
...
control flow).
Table~\ref{tab:bench} shows per-category results on the original 202 pairs;
Table~\ref{tab:bench-expanded} shows results on the 150 expanded pairs.

\begin{table}[t]
\centering
\caption{Benchmark accuracy by category (202 pairs = 52 core + 150 scaled).}\label{tab:bench}
\begin{tabular}{@{}lrrrl@{}}
\toprule
\textbf{Category} & \textbf{Pairs} & \textbf{Correct} & \textbf{Accuracy} & \textbf{Notes} \\
\midrule
arithmetic       & 49  & 46 & 93.9\% & Overflow, wrapping, saturation \\
bitwise          & 29  & 25 & 86.2\% & AND, OR, XOR, popcount \\
cast             & 13  & 13 & 100\%  & Widening, narrowing, sign \\
compound         & 12  & 12 & 100\%  & Multiple divergence classes \\
control\_flow    & 11  & 11 & 100\%  & Branches, ternary \\
division         & 21  & 19 & 90.5\% & Div-by-zero, INT\_MIN/$-1$ \\
shift            & 17  & 17 & 100\%  & Over-width, negative \\
real\_patterns   & 26  & 26 & 100\%  & CRC, hash, alignment \\
error\_handling  &  8  &  4 & 50.0\% & errno vs.\ Result \\
loops            &  8  &  2 & 25.0\% & Bounded iteration (BMC) \\
memory           &  4  &  0 &  0\%   & Pointer patterns \\
string           &  4  &  0 &  0\%   & String operations \\
\midrule
\textbf{Total}   & \textbf{202} & \textbf{175} & \textbf{86.6\%} & \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Expanded benchmark: 150 new pairs across 8 categories.}\label{tab:bench-expanded}
\begin{tabular}{@{}lrl@{}}
\toprule
\textbf{Category} & \textbf{Pairs} & \textbf{Notes} \\
\midrule
struct           & 20 & Field access, nested, overflow \\
enum             & 20 & switch/match, Option, Result \\
float            & 15 & NaN, rounding, conversion \\
c2rust           & 20 & Real transpiler patterns \\
iterator         & 15 & for-loop vs.\ iterator chains \\
cast             & 15 & Widening, narrowing, chain \\
compound         & 15 & Ternary, bitwise, arithmetic \\
control\_flow    & 10 & goto, do-while, labeled break \\
\midrule
...
\begin{axis}[
    ...
    symbolic x coords={arith,bitwise,cast,compound,ctrl,div,shift,real,err,loops,mem,string},
    xtick=data,
...
\addplot[fill=blue!60] coordinates {
    (arith,93.9) (bitwise,86.2) (cast,100) (compound,100) (ctrl,100)
    (div,90.5) (shift,100) (real,100) (err,50.0) (loops,25.0)
    (mem,0) (string,0)
};
\end{axis}
\end{tikzpicture}
\caption{Per-category accuracy on the 202-pair benchmark.  Categories in
         $\mathcal{F}$ (arithmetic through real patterns) achieve
         $\geq$86\%; categories requiring pointer/heap features outside
         $\mathcal{F}$ (memory, strings) achieve 0\%.
         Loop analysis uses BMC at depth $K{=}32$.}\label{fig:bench-bar}
\end{figure}

The oracle achieves 100\% accuracy on 5 of 12 categories and $\geq$86\% on
all categories within the supported fragment $\mathcal{F}$.  The 0\% accuracy
on memory and string categories reflects fundamental parser limitations for
pointer-based and string operations, which are outside $\mathcal{F}$.
Error handling (50\%) is partially supported: simple error-code patterns work,
...
Average verification time is 0.8 seconds per pair.  The full 202-pair suite
completes in under 3 minutes on a single core.
...
We run the CEGAR loop (Algorithm~\ref{alg:cegar}) on 30 C functions using
GPT-4.1-nano with $k = 5$ iterations.  Table~\ref{tab:cegar} summarizes the
results.

\begin{table}[t]
\centering
\caption{CEGAR experiment results (30 functions, GPT-4.1-nano,
...
\midrule
Total functions              & 30 \\
CEGAR converged              & 5/30 (16.7\%) \\
\quad Correct on first try   & 5/30 (16.7\%) \\
\quad Repaired after $\geq$1 iteration & 0/30 (0\%) \\
Remained divergent           & 25/30 (83.3\%) \\
\midrule
Naive retry converged        & 5/20 (25.0\%) \\
\quad (same 5 functions)     & \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{axis}[
    ...
    ymin=0, ymax=40,
    ytick={0,10,20,30},
    symbolic x coords={CEGAR,Naive Retry},
    xtick=data,
    x tick label style={font=\small},
    nodes near coords,
    nodes near coords style={font=\small},
...
\addplot[fill=red!50] coordinates {(CEGAR,16.7) (Naive Retry,25.0)};
\end{axis}
\end{tikzpicture}
\caption{CEGAR convergence vs.\ naive retry.  Both converge on the same
         5 functions (all first-try correct).  Naive retry's higher rate
         is an artifact of the smaller denominator (20 vs.\ 30).}\label{fig:cegar-bar}
\end{figure}

\paragraph{Critical observation.}
All 5 converged functions were correct on the \emph{first} LLM attempt---the
CEGAR loop provided no additional repair.  For the 25 functions with bugs,
zero were repaired despite receiving exact counterexamples with concrete
divergent inputs.  The naive retry baseline converges on the same 5 functions,
confirming that the CEGAR feedback does not improve repair rates.

\subsection{Bug Taxonomy and Repairability}\label{sec:eval:taxonomy}

We classify the 30 CEGAR functions by bug type:

\begin{table}[t]
\centering
\caption{Bug classification and LLM repairability.}\label{tab:taxonomy}
\begin{tabular}{@{}lccl@{}}
\toprule
\textbf{Bug Class} & \textbf{Count} & \textbf{Repaired} & \textbf{Difficulty} \\
\midrule
Undefined behavior   &  6 & 0 (0\%)  & Hard \\
Other output mismatch &  9 & 0 (0\%)  & Medium \\
No bug (first-try correct) & 15 & 5 (33\%) & Easy \\
\bottomrule
\end{tabular}
\end{table}
...
\item[Easy (casts, reinterpretation):] Functions that the LLM translates
     correctly on the first attempt.  These involve straightforward type
     conversions where Rust semantics closely match C.

\item[Medium (shifts, non-UB mismatches):] The LLM produces incorrect
     translations with non-UB output mismatches (e.g., wrong shift direction,
     incorrect masking).  The oracle provides exact counterexamples, but the
     LLM fails to apply the repair.

\item[Hard (overflow, UB):] Divergences rooted in C undefined behavior.
     No Rust translation can simultaneously match C semantics on well-defined
     inputs and replicate C's UB behavior.  The oracle correctly identifies
     these as fundamental semantic gaps.
\end{description}

\subsection{The Negative Result}\label{sec:eval:negative}
...
\addplot[fill=orange!60] coordinates {(UB bugs,0) (Other bugs,0) (No bug,33.3)};
\end{axis}
\end{tikzpicture}
\caption{LLM repair rate by bug class.  GPT-4.1-nano fails to repair any
         bug, whether UB-related or not.  The 33\% ``repair'' rate for
         ``no bug'' reflects first-try correct translations that the CEGAR
         loop verifies without needing repair.}\label{fig:repair-bar}
\end{figure}

The negative result is itself a contribution.  We demonstrate that:

\begin{enumerate}[leftmargin=*,nosep]
\item The CEGAR loop provides \emph{correct} counterexamples (verified by
      re-executing both programs on the counterexample inputs).
...
oracle = VerificationOracle(timeout=10)
...
print(result.time_seconds)     # float

if result.verdict == "divergent":
    cex = result.counterexample
    print(f"Inputs: {cex.inputs}")
    print(f"C output: {cex.c_output}")
    print(f"Rust output: {cex.rust_output}")
...
class VerificationResult:
    ...
    time_seconds: float
    smt_stats: Optional[SMTStats]

@dataclass
class CounterExample:
    inputs: Dict[str, int]
    c_output: Union[int, str]   # str for UB description
    rust_output: Union[int, str]
