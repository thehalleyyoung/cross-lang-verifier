...
On a benchmark of \textbf{202 function pairs} (52 core + 150 scaled) across
12 categories, \semrec{} achieves \textbf{86.6\% overall accuracy} (175/202),
with 100\% accuracy on cast, compound, control-flow, shift, and real-world
patterns.  Comparing the oracle against differential testing on the 52 core
...
\item \textbf{Source-level divergence detection.}
      ...
      achieving 86.6\% accuracy on 202 pairs (\S\ref{sec:eval:bench}).
...
functions, integer/bitvector arithmetic, bounded loops with $K \leq 32$
unrollings):
\[
\Pi(f_C, f_R) = \UNSAT{}
...
(over-approximating C's ``anything can happen'' semantics).  If the product
program $\Pi$ is unsatisfiable, no well-defined input distinguishes $f_C$
from~$f_R$.
\end{proof}
...
\item Control flow: if/else, bounded for/while loops
\item Type casts: widening, narrowing, sign change
\end{itemize}

\noindent\textbf{Limitations.}
Pointers, heap allocation, structs, strings, floating point,
...
Our benchmark comprises \textbf{202 function pairs} (52 core pairs manually
curated + 150 systematically scaled pairs) across 12 categories.
...
loops            &  8  &  2 & 25.0\% & Bounded iteration \\
...
\caption{Per-category accuracy on the 202-pair benchmark.  Categories in
         ...
         $\geq$86\%; categories requiring features outside $\mathcal{F}$
         (memory, strings) achieve 0\%.}\label{fig:bench-bar}
\end{figure}
...
analysis.  Loops (25\%) are limited by the unrolling bound $K=32$.
...
On 202 function pairs, \semrec{} achieves 86.6\% accuracy, with 100\% on
five categories.  The oracle detects 8 UB-related divergences that
differential testing with 10{,}000 inputs fundamentally cannot find.
