...
help:
	...
	@echo "  leaderboard-check assert public leaderboard artifacts and scorer reproduce"
	...
	@echo "  vscode-extension-check assert the VS Code LSP wrapper compiles and packages"
	...
	@echo "  launch-check      validate the launch/community packet against the live demo"
	@echo "  ecosystem-check   enforce v1 API/SemVer snapshot and cargo shim stability"
...
leaderboard-check:
	...
	$(PYTHON) -m experiments.leaderboard.run --check
...
github-action-check:
	$(PYTHON) -m pytest tests/test_github_action.py -q
...
vscode-extension-check:
	$(PYTHON) -m pytest tests/test_vscode_extension.py tests/test_ub_oracle.py -k vscode -q
...
ecosystem-check:
	$(PYTHON) -m pytest \
		tests/test_ub_oracle.py::test_ecosystem_v1_api_surface_intact \
		tests/test_ub_oracle.py::test_ecosystem_generates_cargo_shim_and_snapshot \
		tests/test_ub_oracle.py::test_ecosystem_api_snapshot_is_fresh \
		tests/test_ub_oracle.py::test_ecosystem_api_snapshot_freshness_catches_stale_file \
		tests/test_ub_oracle.py::test_ecosystem_cargo_subcommand_matches_library \
		-q
...
ci: guard cold-start-ci reproduce-check matrix-check cex-quality-check perf-check repro-hardening-check redteam-check verified-check soundness-check github-action-check test-ub
	@echo "ci: PASSED"
